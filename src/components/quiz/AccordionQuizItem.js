...
const AccordionQuizItem = ({ id, title, jumlah, isDone, nilai }) => {
  if (isDone) {
    return (
      <View onTouchEndCapture={()=> router.push(`/quiz/${id}`)} className=' rounded-3xl mt-2 border-[#0EDE16] bg bg-white border-2 border-solid pl-6 pr-3 py-2  flex w-full flex-row justify-between items-center'>
        <View className='flex-col flex w-[70%]'>
          <Text style={styles.bold} className='text-lg'>Quiz {id} - {title}</Text>
